...
export class ChatGPTAPITURBOStream {
   ...
   * you can use `opts.onConversationResponse` or use the `ChatGPTAPITURBOStream.getConversation`
   * helper.
   *
   * Set `debug: true` in the `ChatGPTAPITURBOStream` constructor to log more info on the full prompt sent to the OpenAI completions API. You can override the `promptPrefix` and `promptSuffix` in `opts` to customize the prompt.
   *
